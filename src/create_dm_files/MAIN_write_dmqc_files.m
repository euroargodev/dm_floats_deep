...
DIR_FTP= C.DIR_DM_CPCOR;    % input files directory
DIR_OW = C.DIR_OW;     % calibration files directory(cal_$flt_name$.mat files)
DIR_OUT = C.DIR_DM_FINAL;   % output files with DMQC corrections are put in this directory
...
if exist(['./create_dm_files/paramlog/' flotteur],'dir')==0
    mkdir(['./create_dm_files/paramlog/' flotteur])
end

log_file = ['./create_dm_files/paramlog/' flotteur '/log_dmqc_' flotteur '.txt'];
...
if exist(['./create_dm_files/paramlog/load_param_dmqc_' flotteur '.mat'],'file')
    
    isw = menu('load_param_dmqc', 'USE EXISTING PARAM', 'ENTER NEW PARAM');
    
    if isw==1
        load (['./create_dm_files/paramlog/load_param_dmqc_' flotteur '.mat'])
    else
