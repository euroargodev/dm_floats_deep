    ...
    switch isw
        ...
        case {'HELP'}
            h=helpdlg({'It is possible to choose',...
                ...
                'if 1),  INST_UNCERTAINTY = 0.004 PSU by default.',...
...
choiceishelp=1;
while choiceishelp==1
    choice = questdlg('How do you want to store calibration information','SCIENTIFIC CALIBRATION SECTION', 'N_CALIB=1', 'N_CALIB=N_CALIB+1','HELP','N_CALIB=N_CALIB+1');
    ...
    switch choice
        ...
        case{'HELP'}
            h=helpdlg({'N_CALIB is a dimension used in the SCIENTIFIC CALIBRATION variables',...
                ...
                '  For deep floats, choose N_CALIB=N_CALIB+1'});
            uiwait(h)
    end
...
choiceishelp=1;
while choiceishelp==1
    choice = questdlg('Has salinity previously adjusted for Cpcor?','Cpcor ADJUSTEMENT', 'NO', 'YES','HELP','YES');
...
save (['./create_dm_files/paramlog/load_param_dmqc_' flotteur '.mat'],'s')
